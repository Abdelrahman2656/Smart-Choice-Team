...
import { Tv } from "../Database"; // ✅ عدل المسار لو مختلف
...
export const startSeedingTv = async () => {
  await dbconnection();

  const amazonData = loadJsonFilesFromDir(
    path.join(process.cwd(), "Database/path/amazonTv")
  );
  ...
  try {
    ...
    const existingAsins = await Tv.find({ asin: { $in: validProducts.map((p) => p.asin) } }).select("asin");
