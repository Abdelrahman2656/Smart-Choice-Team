...
import { Tablet } from "../Database"; // ✅ عدل المسار لو مختلف
...
export const startSeedingTablet = async () => {
  await dbconnection();

  const amazonData = loadJsonFilesFromDir(
    path.join(process.cwd(), "Database/path/amazonTablet")
  );
  ...
  try {
    ...
    const existingAsins = await Tablet.find({ asin: { $in: validProducts.map((p) => p.asin) } }).select("asin");
    ...
    await Tablet.bulkWrite(productOps);
